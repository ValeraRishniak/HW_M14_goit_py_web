...
class TestAsync(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_search_contacts(self):
        expected_contacts = []#[Contact(email = "test@test.com"),Contact(email = "test@test.com"),Contact(email = "example@test.com"),Contact(),]
    ...
    #     expected_contacts = [Contact(birthday = date(year=1991, month=8, day=24)),Contact(birthday = '1999-10-26'),Contact(birthday = '1999-10-27'),Contact(birthday = '1999-10-28'),]
    #     self.session.query().filter().all.return_value = expected_contacts
    #     result = await get_birthday_per_week(days=5, user=self.user, db=self.session,)
    #     self.assertEqual(result, expected_contacts)
