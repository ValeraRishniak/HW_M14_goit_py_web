...
async def get_birthday_per_week(days: int, user: User, db: Session) -> Contact:
    ...
    for contact in all_contacts:
        # if timedelta(0) <= (datetime.strptime(str(contact.birthday).replace(str(contact.birthday.year), str(datetime.now().year)), "%Y-%m-%d")) - datetime.now()  <= timedelta(days):
        #     # x = (datetime.strptime(str(birthday).replace(str(birthday.year), str(datetime.now().year)), "%Y-%m-%d") - datetime.now()).days
        #     response.append(contact)
        # if timedelta(days=0) <= (datetime.strptime((str(contact.birthday).replace(str(int(contact.birthday.year)), str(int((datetime.now()).year)))),'%Y-%m-%d') - datetime.now()) <= datetime.now()+timedelta(days=7):
        #     response.append(contact)
